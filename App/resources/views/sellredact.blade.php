...
@section('content')
    <div class="text-gray-700 text-2xl font-semibold my-5">Запрос номер {{$item->id}}
        <a class="text-blue-500 font-normal" href="{{\Illuminate\Support\Facades\URL::to('/sell/view/'.$item->id)}}">
            ({{\Illuminate\Support\Facades\URL::to('/sell/view/'.$item->id)}})
        </a>
    </div>
    <form action="{{URL::to('sell/view/'.$item->id.'/redact/delete')}}" method="POST">
        @csrf
